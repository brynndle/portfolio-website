...
## Featured offering strip (15a + 17a label)

Three columns, `1.9fr | 1.1fr | auto`:

1. **What it is** — the widest column, and deliberately so. Black label tag, the
   offer name, then two paragraphs that explain the service in plain language.
   Earlier versions led with the 01/02/03 process, which made the strip look
   busy while never saying what you were buying. Prose wins the space.
2. **How it works** — the process, demoted: a `How it works` micro-label and
   three hairline-ruled lines, plum numerals, no descriptions.
3. **Action** — `bg-slate` (#eceef0, cooler than paper so it separates), the
   availability line in green, blue CTA, format footnote.

The label is a tag, not colored text: `bg-ink` + `text-paper`, `px-2 py-[5px]`,
mono 12px, `tracking-[0.18em]`. It's the first thing the eye lands on and then
gets out of the way — no green dot, no icon. Reuse this tag for any other
"category" label; don't invent a second badge style.

Green is availability only. Plum (`text-accent`) is the numerals. Blue is the
action. Below `lg` the card stacks to one column and the vertical dividers
become bottom borders.
