...
## Featured offering strip (15a / 17a)

- `components/FeaturedOffering.astro` — rebalanced to `1.9fr | 1.1fr | auto`: the
  explanation gets the widest column, the 01/02/03 process is now a compact
  ruled list, and "Featured Offering" is a black tag (`bg-ink` / `text-paper`,
  `px-2 py-[5px]`) instead of green text with a dot.
- Copy for the pitch lives in the `pitch` prop (default in the component); the
  three process lines are the `steps` array in its frontmatter.
- No token or CSS changes needed — uses existing `ink`, `paper`, `slate`,
  `accent`, `green`, `rule` tokens.
