...
- `components/Hero.astro` — 14b dot-field left column (corner-faded), orchid right panel with mist grain.
- `global.css` — new `--color-panel` token (light `#8a4fd1`, dark `#6b3aa8`); `--color-paper-2` retuned to `#efeadd`; brighter `--color-green` `#00875a`.
