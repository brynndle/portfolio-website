...
Left column: headline + lede vertically centered over a field of thin outlined
shapes (circles, a rotated square, a quarter-round, one hairline) drawn as
absolutely-positioned divs, clipped by `overflow-hidden`. Strokes are
`rgb(20 22 26 / 0.14–0.16)`; one circle uses `border-panel/30` so the accent
appears once. Decorative only — `aria-hidden`, `pointer-events-none`.
